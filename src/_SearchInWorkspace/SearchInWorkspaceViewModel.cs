                ...
                if (searchTerm != "" && searchTerm != " ")
                {
                    ...
                    if (searchInNicknames || searchInOriginalNames || searchInCategories || searchInDescriptions)
                    {
                        foreach (NodeModel node in readyParams.CurrentWorkspaceModel.Nodes)
                        {
                            ...
                            if (rawScore > 0)
                            {
								...
								unorderedResults.Add(new ObjectInWorkspace(node.Name.Abbreviate(), node.GUID.ToString(), weightedScore, toolTip, "Node"));
                            }
                        }
                    }
                    if (searchInNotes)
                    ...
                    if (searchInAnnotations)
                    {
                        foreach (AnnotationViewModel anno in viewModel.CurrentSpaceViewModel.Annotations)
                        {
                            ...
                            weightedScore = rawScore / (10d + searchTermParts.Length);
							if (rawScore > 0) { unorderedResults.Add(new ObjectInWorkspace(anno.AnnotationText.Abbreviate(), anno.AnnotationModel.GUID.ToString(), weightedScore, "Search score: " + weightedScore.ToString() + "\n\n" + anno.AnnotationText, "Group")); }
                        }
                    }
