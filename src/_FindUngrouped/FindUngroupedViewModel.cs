...
namespace Monito
{
    class FindUngroupedViewModel : NotificationObject, IDisposable
    {
        ...
        public ICommand FixUngroupedCommand { get; set; }
		...
		public FindUngroupedViewModel(ReadyParams p, DynamoViewModel vm)
        {
            ...
            FixUngroupedCommand = new DelegateCommand(OnFixUngroupedClicked);
        }
        ...
        public string CurrentUngroupedMsg
        {
            get
            {
				...
				{
					currentUngroupedMsg = batchProcessResults;
					batchProcessResults = "";
				}
				else if (currentUngrouped.Count > 0) { currentUngroupedMsg = "All ungrouped nodes and text notes in current workspace:"; }
        ...
        public ObservableCollection<ObjectInWorkspace> CurrentUngrouped
        {
            get
            {
                ...
                foreach (var node in viewModel.CurrentSpaceViewModel.Nodes)
                {
                    ...
                    {
						unorderedUngrouped.Add(new ObjectInWorkspace(node.Name.Abbreviate(), node.NodeModel.GUID.ToString(), 0, "", "Node"));
                    }
                }
